...
def create_performance_dashboard(analyzer):
    ...
    # NEP 2020 Transition calculations (Apply backwards dynamically if DB lacks them)
    if 'accreditation_status' not in df.columns:
        df['accreditation_status'] = df['performance_score'].apply(
            lambda x: 'Accredited' if x >= 6.0 else ('Awaiting Accreditation' if x >= 5.0 else 'Not Accredited')
        )
        df['excellence_level'] = df['performance_score'].apply(
            lambda x: 'Level 5 (Global)' if x >= 9.0 else ('Level 4 (National)' if x >= 8.0 else ('Level 3 (Established)' if x >= 7.0 else ('Level 2 (Progressing)' if x >= 6.0 else 'Level 1 (Developing)')))
        )
    ...
    with col1:
        accredited_rate = (current_year_data['accreditation_status'] == 'Accredited').mean()
        st.metric("Binary Accredited Rate", f"{accredited_rate:.1%}")
    
    with col2:
        top_level = current_year_data['excellence_level'].mode()[0]
        st.metric("Avg Excellence Level", top_level)
    ...
    with col1:
        fig1 = px.histogram(
            current_year_data, 
            x='excellence_level',
            title="NEP 2020 Excellence Level Distribution",
            color='excellence_level',
            opacity=0.8
        )
        fig1.update_layout(xaxis_title="Excellence Level", yaxis_title="Count")
    ...
    with st.expander("Calculate Custom Performance Score"):
        ...
        if st.button("Calculate Performance Score"):
            ...
            # Pre-existing unresolved variables patched with safe fallbacks
            nirf_ranking = nirf_rank
            phd_faculty_ratio = 0.6
            faculty_count = 100
            research_grants_amount = 0
            patents_filed = 0
            digital_infrastructure_score = digital_infrastructure
            library_volumes = 10000
            campus_area = 50
