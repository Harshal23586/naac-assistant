...
def create_document_analysis_module(analyzer):
    ...
    # AI Content Verification Section
    st.markdown("---")
    st.subheader("🔍 AI-Based Content Verification")
    st.info("Upload physical documents to deeply verify their content against institutional metrics.")
    
    uploaded_files = st.file_uploader(
        "Upload files for RAG verification (PDF, DOCX)",
        accept_multiple_files=True,
        key="doc_verification_uploader"
    )
    ...
    if st.button("🤖 Analyze Document Sufficiency", type="primary"):
        if uploaded_files:
            with st.spinner("Analyzing document content with AI..."):
                verification_result = analyzer.verify_document_authenticity(uploaded_files)
                
                if verification_result['verified']:
                    st.success("✅ Documents verified successfully! The content contains valid parseable academic data.")
                    st.metric("AI Confidence Score", f"{verification_result['confidence_score']:.2%}")
                    
                    # Log true verification state internally for approval engine
                    st.session_state.ai_document_verified = True
                else:
                    st.error("❌ High Risk: Documents appear invalid, empty, or unparseable. Approval will be downgraded.")
                    st.session_state.ai_document_verified = False
                    
                with st.expander("View AI Extracted Metrics & Raw Text"):
                    st.json(verification_result['extracted_metrics'])
