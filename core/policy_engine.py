...
class StatutoryPolicyEngine:
    ...
    def evaluate_compliance(metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Rule: Strict adherence to <= 20.0 for basic approval parameters
        if sfr <= 20.0:
        ...
        # Rule: At least 40% of faculty should hold PhDs for generalized excellence
        if phd_ratio >= 0.40:
        ...
        # Rule: Encourages community outreach, interdisciplinary exposure
        if community >= 2:
        ...
        # Rule: Financial stability score needs to handle minimum operations
        if financial >= 6.0:
