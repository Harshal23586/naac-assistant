...
from modules.rag_core import DocumentFormValidator
...
class InstitutionalAIAnalyzer:
            ...
            try:
                ...
                if not st.session_state.rag_initialized:
                    with st.spinner("🔄 Loading AI Document Analysis Components..."):
                        ...
                        if hasattr(self.rag_extractor, 'embedding_model') and getattr(self.rag_extractor, 'embedding_model', None) is not None:
    ...
    def analyze_documents_with_rag(self, institution_id: str, uploaded_files: List) -> Dict[str, Any]:
        ...
        if self.rag_extractor:
            return self.rag_extractor.analyze_documents_with_rag(institution_id, uploaded_files)  # type: ignore
        else:
            return {"error": "RAG system not initialized", "analysis": {}}

    def verify_document_authenticity(self, uploaded_files: List, expected_metrics: Optional[Dict] = None) -> Dict:
        """Verify document using RAG Form Validator"""
        if not hasattr(self, 'document_validator'):
            self.document_validator = DocumentFormValidator()  # type: ignore
            
        doc_data = self.document_validator.extract_document_data(uploaded_files)  # type: ignore
        
        if not expected_metrics:
            return {
                "verified": True if doc_data.get('extracted_text', '').strip() else False,
                "confidence_score": 1.0 if doc_data.get('extracted_text', '').strip() else 0.0,
                "extracted_metrics": doc_data
            }
            
        comparison = self.document_validator.compare_with_form_data(doc_data, expected_metrics)  # type: ignore
